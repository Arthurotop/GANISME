...
class ArtGANTrainer:
    ...
    def _init_dcgan(self) -> None:
        ...
        # Adjust generator for different image sizes
        if self.image_size == 64:
            ...
            self.generator = nn.Sequential(
                nn.ConvTranspose2d(
                    self.latent_dim, feature_maps_gen * 16, 4, 1, 0, bias=False
                ),
                nn.BatchNorm2d(feature_maps_gen * 16),
                nn.ReLU(True),
                nn.ConvTranspose2d(
                    feature_maps_gen * 16, feature_maps_gen * 8, 4, 2, 1, bias=False
                ),
                nn.BatchNorm2d(feature_maps_gen * 8),
                nn.ReLU(True),
                nn.ConvTranspose2d(
            ...
            raise ValueError(f"Unsupported image size: {self.image_size}. Only 64 and 128 are supported.")
        ...
        # Adjust discriminator for different image sizes
        if self.image_size == 64:
            self.discriminator = nn.Sequential(
            ...
            self.discriminator = nn.Sequential(
                ...
                nn.BatchNorm2d(feature_maps_disc * 8),
                nn.LeakyReLU(0.2, inplace=True),
                sn_conv(feature_maps_disc * 8, feature_maps_disc * 16, 4, 2, 1),
                nn.BatchNorm2d(feature_maps_disc * 16),
                nn.LeakyReLU(0.2, inplace=True),
                sn_conv(feature_maps_disc * 16, 1, 4, 1, 0),
            )
        else:
            raise ValueError(f"Unsupported image size: {self.image_size}. Only 64 and 128 are supported.")
